...
* first of all, CMake is not used (directly) to build VST 3 plugins. CMake is used to generate a platform dependent project (ex: XCode project for Mac, Visual Studio project for Windows). This project (hence the build framework associated with it) is then used to build the examples provided in the sdk as well as compile the SDK itself (multiple libraries) and some tools.

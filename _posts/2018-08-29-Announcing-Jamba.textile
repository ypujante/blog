...
The "Jamba Sample Gain":https://github.com/pongasoft/jamba-sample-gain plugin is the documentation plugin for this framework and demonstrates most of the features the framework has to offer.

The Jamba framework also provides a simple command line executable which creates a fully functionning (Jamba enabled) plugin: check "Generating a blank plugin":https://github.com/pongasoft/jamba/blob/master/README.md#generating-a-blank-plugin.
...
Jamba offers a simple command line executable which creates a fully functionning (Jamba enabled) plugin: check "Generating a blank plugin":https://github.com/pongasoft/jamba/blob/master/README.md#generating-a-blank-plugin.
...
Although there are many ways to accomplish this, <code>jamba.cmake</code> offers an integration with Google Test as a starting point. When using the "Generating a blank plugin":https://github.com/pongasoft/jamba/blob/master/README.md#generating-a-blank-plugin command, the plugin comes with a single unit test example so that it is easy to add more tests without having to deal with the details.
...
By including <code>jamba.cmake</code>, Jamba generates shell/bat convenient scripts:

* <code>build.sh</code> => compiles the plugin and generates the <code>.vst3</code> bundle
* <code>test.sh</code> => runs the unit tests
* <code>validate.sh</code> => runs the validator tool which executes some vst specific tests against the plugin and make sure it complies with VST3
* <code>edit.sh</code> => in Debug mode, the plugin can always be edited by right clicking in the UI but it can be pretty time consuming to generate the plugin, install it, start a DAW in order to simply edit the UI. This shell script starts the convenient tool (which comes with the VST3 SDK) to edit the plugin outside a DAW
* <code>install.sh</code> => copies the plugin in its appropriate folders (on macOS)

h4. How do I build build for macOS and Windows 10?
...
* <code>CMakeLists.txt</code> which contains the list of sources (you need to add your own source files in the <code>vst_sources</code> list). "Example":https://github.com/pongasoft/jamba-sample-gain/blob/v1.0.1/CMakeLists.txt#L37.
* <code>CMakeLists.txt</code> which contains the list of resources (graphics) for the UI for macOS (section <code>smtg_add_vst3_resource</code>). "Example":https://github.com/pongasoft/jamba-sample-gain/blob/v1.0.1/CMakeLists.txt#L67.
* <code>resource/XXX.rc</code> which contains the list of resources (graphics) for the UI for windows. "Example":https://github.com/pongasoft/jamba-sample-gain/blob/v1.0.1/resource/JSGain.rc.
